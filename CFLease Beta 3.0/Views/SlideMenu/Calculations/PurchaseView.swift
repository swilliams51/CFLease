...
struct PurchaseView: View {
    ...
    @FocusState private var buyrateIsFocused: Bool
    ...
    var body: some View {
        ...
        .onAppear{
            if self.myLease.purchaseFee.toDecimal() != 0.0 {
                ...
                let decFeePaid: Decimal = self.myLease.amount.toDecimal() * 0.02
    ...
    func getMaxBuyRate() -> Decimal {
        return self.myLease.interestRate.toDecimal()
    }
    
    func decimalpadIsActive() -> Bool {
        var padIsActive: Bool = false
        if feeIsFocused == true || buyrateIsFocused == true {
            padIsActive = true
        }
    ...
    func updateForNewFee() {
        ...
        if isAmountValid(strAmount: feePaid, decLow: 0.0, decHigh: getMaxPremium(), inclusiveLow: true, inclusiveHigh: true) == false {
            ...
            if feePaid.toDecimal() < 1.0 {
                self.feePaid = percentToPremiumAmount(percent: feePaid)
            }
            self.buyRate = self.myLease.getBuyRate(aPremiumPaid: feePaid, afterLesseeFee: false)
        }
    }
...
struct Purchase_Previews: PreviewProvider {
    ...
    static var previews: some View {
        ...
        PurchaseView(myLease: Lease(aDate: today()), isDark: .constant(false))
...
let alertValidFeeAmopunt: String = "The fee must be equal to or greater than zero and less than the undiscounted sum of the total payments less lease amount!!"
