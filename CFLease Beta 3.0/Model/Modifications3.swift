...
func getGroupsAfterModDate(aSchedule: Groups, modDate: Date) -> Groups {
    ...
    while y < aSchedule.items.count {
        if aSchedule.items[y].timing == PaymentTiming.arrears || aSchedule.items[y].timing == PaymentTiming.equals {
            ...
            if aSchedule.items[y].startDate >= modDate {
                if aSchedule.items[y].startDate == modDate {
                    let newGroup = Group(aAmount: "0.0", aEndDate: aSchedule.items[y].endDate, aLocked: true, aNoOfPayments: 1, aStartDate: modDate, aTiming: .advance, aType: .payment, aUndeletable: false)
                    tempGroups.items.append(newGroup)
                    interimAdded = true
                } else {
                    if interimAdded == false {
                        let newGroup = Group(aAmount: "0.0", aEndDate: aSchedule.items[y].startDate, aLocked: true, aNoOfPayments: 1, aStartDate: modDate, aTiming: .advance, aType: .payment, aUndeletable: false)
                        tempGroups.items.append(newGroup)
                        interimAdded = true
                    }
                    tempGroups.items.append(aSchedule.items[y])
...
func groupsToSchedule(aGroups: Groups, aPayPerYear: Frequency, aReferDate: Date, aEOM: Bool) -> Groups {
    ...
    for x in 0..<wrkGroups.items.count {
        if wrkGroups.items[x].noOfPayments == 1 {
            let myGroup = Group(aAmount: wrkGroups.items[x].amount, aEndDate: wrkGroups.items[x].endDate, aLocked: wrkGroups.items[x].locked, aNoOfPayments: 1, aStartDate: wrkGroups.items[x].startDate, aTiming: wrkGroups.items[x].timing, aType: wrkGroups.items[x].type, aUndeletable: wrkGroups.items[x].undeletable)
            ...
            while iCounter <= wrkGroups.items[x].noOfPayments {
                ...
                let myGroup = Group(aAmount: strAmount, aEndDate: currEnd, aLocked: aLocked, aNoOfPayments: 1, aStartDate: currStart, aTiming: aTiming, aType: aType, aUndeletable: aUndeletable)
                myGroups.items.append(myGroup)
                currStart = currEnd
                iCounter = iCounter + 1
            }
...
func modifiedLease(aLease: Lease, modDate: Date) -> Lease {
    ...
    //cetermine start and end date for the payoff group to be added
    ...
    let myGroup: Group = Group(aAmount: prepaidAmount.toString(decPlaces: 5), aEndDate: dateEnd, aLocked: true, aNoOfPayments: 1, aStartDate: dateStart, aTiming: intTiming, aType: .payment, aUndeletable: true)
...
func eboLease(aLease: Lease, modDate: Date, rentDueIsPaid: Bool) -> Lease {
    ...
    //Adjust last rent payment amount if not paid in addirion to EBO
    ...
    let residualGroup = Group(aAmount: strEBOAmount, aEndDate: aDateEnd, aLocked: false, aNoOfPayments: 1, aStartDate: aDateEnd, aTiming: .equals, aType: .residual, aUndeletable: false)
