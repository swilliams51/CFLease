...
class Lease: ObservableObject {
    ...
    @Published var lesseePaidFee: String
    @Published var paymentsPerYear: Frequency
    @Published var purchaseFee: String
    ...
    //MARK: - Initialization
    init(amt: String, baseCommence: Date, term: Int, child: Int = -1, EOM: Bool, firstAnnual: Date, funding: Date, intCalcMethod: DayCountMethod, rate: String, paidFees: String, payPerYear: Frequency, buyFee: String) {
        amount = amt
  ...
  //MARK: - Create Payments
    ...
    func implicitRate() -> Decimal {
        ...
        if lesseePaidFee.toDecimal() > 0.00 {
            let decAmount: Decimal = tempLease.amount.toDecimal() - lesseePaidFee.toDecimal()
            tempLease.amount = decAmount.toString()
            tempLease.solveForRate()
            strImplicit = tempLease.interestRate.toTruncDecimalString(decPlaces: 5)
        }
    ...
    func getTotalCashflows() -> Cashflows {
        let myFees = Cashflows(lesseePaidFees: lesseePaidFee.toDecimal(), datePaid: fundingDate)
    ...
    func getTotalInterest() -> Decimal {
        let decTotalCashIn: Decimal = getTotalRents() + getTotalResidual()
        return decTotalCashIn - amount.toDecimal()
    }
    ...
    func eboExists() -> Bool {
        ...
        let eboAmount = earlyBuyOut!.amount.toDecimal()
        let parValue = getParValue(askDate: earlyBuyOut!.exerciseDate, rentDueIsPaid: earlyBuyOut!.rentDueIsPaid).toString()
        if abs(eboAmount - parValue.toDecimal()) < 10.0 {
            return false
        }
        return true
    }
    ...
    func terminationsExist() -> Bool {
        ...
        if testRentRate == true && testResidualRate == true {
            return false
        } else {
            return true
        }
