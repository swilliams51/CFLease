...
//  CFLease Beta 3.0
//
//  Created by Steven Williams on 12/7/21.
//
...
extension Lease {
    ...
    func getBuyRate(aPremiumPaid: String, afterLesseeFee: Bool) -> String {
        let tempLease: Lease = self.clone()
        if afterLesseeFee == true {
            tempLease.amount = (tempLease.amount.toDecimal() - tempLease.lesseePaidFee.toDecimal()).toString(decPlaces: 5)
        }
        var strBuyRate: String = tempLease.interestRate
        
        if aPremiumPaid.toDecimal() > 0.00 {
            let decFeePaid: Decimal = aPremiumPaid.truncateAmount(decPlaces: 4).toDecimal()
            ...
            strBuyRate = tempLease.interestRate
        }
       
        return strBuyRate.toTruncDecimalString(decPlaces: 4)
    }
...
extension Lease {
    ...
    func getEBOAmount (aLease: Lease, bpsPremium: Int, exerDate: Date, rentDueIsPaid: Bool) -> String {
        ...
        if bpsPremium > 0 {
            ...
            m_EBOLease.solveForUnlockedPayments()
    ...
    func eboYield(aLease: Lease, rentDueIsPaid: Bool, withLesseeFee: Bool) -> Decimal {
        ...
        if withLesseeFee == true {
            m_EBOLease.amount = (m_EBOLease.amount.toDecimal() - aLease.lesseePaidFee.toDecimal()).toString()
        }
        m_EBOLease.solveForRate()
...
extension Lease {
    ...
    func getMaxResidualGuaranty (discountRate: Decimal) -> Decimal {
        let pvOfMaxGty: Decimal =  (0.8995 * amount.toDecimal()) - lesseePaidFee.toDecimal() - getPVOfRents(discountRate: discountRate)
    ...
    func getTotalObligation(discountRate: Decimal, residualGuaranty: Decimal) -> Decimal {
        return lesseePaidFee.toDecimal() + getPVOfRents(discountRate: discountRate) + getPVOfResidualGuaranty(discountRate: discountRate, residualGuaranty: residualGuaranty)
    }
...
extension Lease {
    ...
    func parValues() -> Cashflows {
        if amortizations.items.isEmpty != false {
            amortizations.items.removeAll()
        }
        setAmortizationsFromCashflow(consolidate: true)
        let myLeaseCFs: Cashflows = Cashflows(aAmortizations: amortizations, returnPayments: false)
        
        for x in 0..<myLeaseCFs.items.count {
            myLeaseCFs.items[x].amount = myLeaseCFs.items[x].amount / amount.toDecimal()
        }
        amortizations.items.removeAll()
