...
//  CFLease Beta 3.0
//
//  Created by Steven Williams on 3/17/22.
//
...
extension Lease {
    func paymentChangeFactors() -> [String] {
        ...
        var changeRate: Decimal = interestRate.toDecimal() + 0.005
        tempLease.interestRate = changeRate.toString(decPlaces: 7)
        tempLease.solveForUnlockedPayments()
        var y: Int = 0
        for x in 0..<tempLease.groups.items.count {
            if tempLease.groups.items[x].type == .payment {
                let newPayment: Decimal = tempLease.groups.items[x].amount.toDecimal()
                let perBPChange: Decimal = (newPayment - arryBase[y]) / 50.0
                arryIncrease.append(perBPChange)
                y += 1
            }
        }
            
        changeRate = interestRate.toDecimal() - 0.005
        tempLease.interestRate = changeRate.toString(decPlaces: 7)
        tempLease.solveForUnlockedPayments()
        y = 0
        for x in 0..<tempLease.groups.items.count {
            if tempLease.groups.items[x].type == .payment {
                let newPayment: Decimal = tempLease.groups.items[x].amount.toDecimal()
                let perBPChange: Decimal = (arryBase[y] - newPayment) / 50.0
    ...
    func eboChangeFactor() -> String {
        ...
        let highFactor = (newEBOAmount1 - baseEBOAmount) / 50.0
        ...
        let lowFactor: Decimal = (baseEBOAmount - newEBOAmount2) / 50.0
